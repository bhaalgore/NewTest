...
int main(int argc, char *argv[])
{
    ...
    QObject::connect(
     &notificationModel, &QAbstractItemModel::rowsInserted,
     [&](const QModelIndex &parent, int first, int last) {
         for (int i = first; i <= last; ++i) {
             ...
             if (object) {
                 ...
                 if (window) {
                     window->setProperty(
                      "title", notificationModel.data(notificationModel.index(i, 0),
                                                      LogicPlugin::NotificationModel::TitleRole));
                     window->setProperty("message", notificationModel.data(
                                                     notificationModel.index(i, 0),
                                                     LogicPlugin::NotificationModel::MessageRole));
                     window->setProperty(
                      "type", notificationModel.data(notificationModel.index(i, 0),
